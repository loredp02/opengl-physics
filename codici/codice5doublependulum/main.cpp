...
btRigidBody* pendulum1RB = nullptr;
btRigidBody* pendulum2RB = nullptr;
...
//per creare un doppio pendolo il punto di ancoraggio del "PRIMO" (quello collegato al cubo) deve essere un corpo rigido, in questo caso creiamo entrambi i pendoli come corpi rigidi
void create_physics_pendulum(){
  //PRIMO
  btCollisionShape* pendulum1Shape = new btBoxShape(btVector3(0.5, 2.0, 0.5));
  float massaPendulum1 = 1.0f;
  btVector3 posPendulum1(5, 5, 0);
  pendulum1RB = createRB(massaPendulum1, posPendulum1, pendulum1Shape);

  //SECONDO
  btCollisionShape* pendulum2Shape = new btBoxShape(btVector3(0.5, 2.0, 0.5));
  float massaPendulum2 = 0.0f; //chiaramente il pendulum2 deve essere uno static o tutto il pendolo cadrà sotto effetto della gravità.
...
void create_physics_constraints(){
  //CREIAMO IL VINCOLO TRA IL PRIMO PENDOLO E IL CUBO
  btVector3 anchorInCube(5,5,0);
  btVector3 anchorInPendulum1(0,0,0);

  btPoint2PointConstraint* pendulum1Constraint = new btPoint2PointConstraint(
        *cubeRB,
        *pendulum1RB,
        anchorInCube,
        anchorInPendulum1
  ...
  btPoint2PointConstraint* pendulum2Constraint = new btPoint2PointConstraint(
        *pendulum1RB,
        *pendulum2RB,
