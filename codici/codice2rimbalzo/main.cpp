...
struct global_struct {
  ...
  const float g = -2; //accelerazione di gravità
...
btRigidBody* createRB(float massa, const btVector3& pos, btCollisionShape* forma) {
  ...
  //andiamo a settare alla creazione dei corpi un "coefficiente di restituzione" che quantifica il rapporto tra la differenza di velocità dei due enti prima e dopo la collisione
  //NB: è il motivo per cui il cubo rimbalza negli urti
...
btRigidBody* staticRB = nullptr; //creiamo un corpo rigido statico su cui colliderà il nostro cubo.
...
void create_physics_static(){
  ...
  float massaStatic = 0.0f; //affinché sia statico deve essere di massa nulla

  
  staticRB = createRB(massaStatic, btVector3(0,-5,0), staticShape); //leghiamo il puntatore al corpo statico
...
void create_scene() {
  ...
  //creiamo il cubo statico
  create_physics_static();
}
